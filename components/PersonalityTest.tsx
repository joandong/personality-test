        ...
        return (
            <div className="results">
                            ...
                            return (
                                <div key={archetype} className={`archetype-result ${scoreLevel}`}>
                                    ...
                                    <div className="interpretation">
                                        <p><strong>核心特质：</strong>{desc.traits}</p>
                                        <p><strong>主要表现：</strong></p>
                                        <ul>
                                            {desc.positive.map((trait, index) => <li key={index}>{trait}</li>)}
                                        </ul>
                                        <p className="score-interpretation">
                                            {score >= 24 ? '这个原型在您的生命中非常活跃，是您性格的重要组成部分。' :
                                             score >= 16 ? '这个原型在您的生命中有适度的影响力，处于平衡状态。' :
                ...
                <div className="result-actions">
                    <button onClick={() => setShowResults(false)} className="restart-btn">重新测试</button>
                </div>
            </div>
        );
    };
        ...
        <div className="container">
            ...
            {!showResults ? (
                <>
                    ...
                    <button className="submit-btn" onClick={handleSubmit}>提交测试</button>
                </>
            ) : renderResults()}
        </div>
    );
